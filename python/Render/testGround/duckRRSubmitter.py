    ...
    def submitRender(self):
       ...
       #check nuke script type to determine render out path
        ...
        if df.updateSavers():
            ...
            #RR commands
            rrCommand = "%s %s -SeqStart %d -SeqEnd %d -SeqStep %d DefaultClientGroup=0~Nuke6 Priority=0~%d SeqDivMIN=0~1 SeqDivMAX=0~1" % (self.rrPath, nukeScriptPath, self.frameStart.value(), self.frameEnd.value(), self.frameStep.value(), self.priority.getValue())
